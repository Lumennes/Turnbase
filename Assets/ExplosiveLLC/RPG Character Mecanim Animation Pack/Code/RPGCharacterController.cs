...
namespace RPGCharacterAnims
{
    ...
    public class RPGCharacterController : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            {
                if (!HandlerExists("Attack")) { return; } // если есть возможность атаковть

                if (CanStartAction("Attack")) // если можно начать атаковать
                {
                    // если есть правое оружие
                    if (hasRightWeapon || rightWeapon == (int)Weapon.Unarmed)
                        StartAction("Attack", new Actions.AttackContext("Attack", "Right")); // left-левой
                }
            }
        }
