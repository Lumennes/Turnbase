...
public class AttackTriggerC : MonoBehaviour {
	...
	[System.Serializable]
	public class LockOnA{
		public bool enable = false;
		public float radius = 3.0f;  //this is radius to checks for other objects
		public float lockOnRange = 2.0f; //this is how far it checks for other objects
		[HideInInspector]
		public Transform lockTarget;
		[HideInInspector]
		public GameObject target;
	}
	public LockOnA autoLockTarget;
	[HideInInspector]
	public bool onLocking = false;
	
	void Awake(){

		GlobalConditionC.language = language;
		Debug.Log(GlobalConditionC.language);
	...
	public void TriggerAttack(){
		if(freeze || atkDelay || Time.timeScale == 0.0f || GetComponent<StatusC>().freeze || GlobalConditionC.freezePlayer){
			return;
		}
	...
	public void TriggerSkill(int sk){
		if(freeze || atkDelay || Time.timeScale == 0.0f || GetComponent<StatusC>().freeze || isCasting || !skill[sk].skillPrefab || GlobalConditionC.freezePlayer){
			return;
		}
	...
	public void GetActivator(GameObject obj , string msg , string btn){
		if(actvateObj && actvateObj.GetComponent<MountControllerC>()){
			//Ignore Other Object when mounting.
			return;
		}
		actvateObj = obj;
		actvateMsg = msg;
